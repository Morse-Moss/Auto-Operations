...
- 占位 `SECRET_KEY` 配合非 loopback 监听会被启动硬闸拒绝：首次部署需在项目根目录 `.env` 或环境变量中设置真实 `SECRET_KEY`（生产另按部署环境配置数据库、CORS、域名和 HTTPS）。
- 服务端下载用户提交的资产 URL 时统一经公网校验（拒绝内网/loopback/云 metadata 地址），Docker 部署默认 `ENVIRONMENT=production`。
...
- 数据库：内网默认 SQLite；服务端数据库兼容能力继续保留，但不作为当前交付目标。
- 运行入口：根目录 `main.py`。
...
py -3 main.py --host 0.0.0.0 --with-frontend
```
...
- 局域网设备：`http://<开发机局域网 IP>:18080`

当前开发和验收均以根目录 `master` 的局域网服务为事实源，标准端口为前端 `18080`、后端 `18081`。
...
- 当前不维护 Railway 或公网部署，后续开发、联调和验收以局域网环境为准。
- 已有服务端数据库、生产安全检查等代码继续保留，避免回退公共运行时质量，但不代表当前承担公网发布承诺。
- 真实账号发布、自动运营和外部模型调用仍需单独授权；仅启动内网服务不会自动开放这些高风险动作。

## 许可与归属

除另有说明外，本项目中由 Morse-Moss 创作并有权授权的原创内容采用 [Apache License 2.0](LICENSE) 许可。

第三方依赖、平台 SDK 或签名资产、打包产物，以及带有独立版权或许可声明的文件，继续适用其各自条款，不因本项目许可证而被重新授权。公开分发前应确认这些材料具备相应授权，或将其替换、排除。

“小红书”等名称和标识的相关权利归其权利人所有；本项目许可证不授予任何商标使用权。
